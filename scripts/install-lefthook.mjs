...
async function main() {
  ...
  let lefthookPackage = null
  try {
    lefthookPackage = (await import('lefthook/package.json', { with: { type: 'json' } })).default
  } catch {
    console.warn('install-lefthook: lefthook is not installed; skipping git hook installation')
  }
  if (lefthookPackage === null || typeof lefthookPackage.bin?.lefthook !== 'string') return
