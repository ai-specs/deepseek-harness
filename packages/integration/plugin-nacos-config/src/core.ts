...
export interface NacosConfigClientOptions {
  /** Nacos server, e.g. http://nacos.internal:8848 */
  server: string
...
function url(base: string, params: Record<string, string>): string {
  ...
  return `${base.replace(/\/+$/, '')}/nacos/v1/cs/configs?${qs}`
}
...
export class NacosConfigClient {
  ...
  private configUrl(dataId: string): string {
    ...
    return url(o.server, {
      dataId,
      group: o.group ?? DEFAULT_GROUP,
      tenant: o.namespace ?? 'dsh',
    })
  }

  /** Fetch one config, update the cache, and return the parsed document. */
  async fetchConfig<T = unknown>(dataId: string): Promise<T | undefined> {
    ...
    if (raw === 'config data not exist') return undefined
    const md5 = contentMd5(raw)
