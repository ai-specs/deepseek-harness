...
describe('web command-line provider', () => {
  ...
  it('rejects the intentionally unsupported all-interfaces host before the consumer activates', async () => {
    ...
    expect(observed.out).toContain('--host 0.0.0.0 is intentionally not supported yet for safety')
    expect(values).toBeUndefined()
    expect(observed.readerConfig).toBeUndefined()
    expect(observed.exits).toEqual([1])
  })

  it('binds the all-interfaces host when the container escape hatch is set', async () => {
    process.env.DSH_WEB_ALLOW_REMOTE_BIND = '1'
    try {
      ...
      delete process.env.DSH_WEB_ALLOW_REMOTE_BIND
    }
  })
})
