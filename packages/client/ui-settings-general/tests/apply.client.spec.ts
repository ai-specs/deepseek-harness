...
import { UserSection } from '../src/client/UserSection.tsx'
...
function expectSeated(c: TestClient): void {
  ...
  expect(ownEntries(c, 'settings.section').map(entry => entry.component)).toEqual([GeneralSection, UserSection])
}
